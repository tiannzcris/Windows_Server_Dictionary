...
export function openEdit(term) { editingId = term.id; const form = dialog("#term-form"); form.dataset.currentImage = term.image || ""; Object.entries({ term: term.term, category: term.category, pronunciation: term.pronunciation || "", shortDefinition: term.shortDefinition || "", extendedDefinition: term.extendedDefinition || "", relatedTerms: (term.relatedTerms || []).join(", "), example: term.example || "" }).forEach(([name, value]) => { form.elements[name].value = value; }); dialog("#term-form-heading").textContent = "Edit Your Term"; dialog("#term-form-submit").textContent = "Save Changes"; dialog("#term-form-error").hidden = true; dialog("#term-form-dialog").showModal(); dialog("#field-term").focus(); }

async function save(event) { event.preventDefault(); const form = event.currentTarget; const input = Object.fromEntries(new FormData(form)); const error = dialog("#term-form-error"); if (!input.term.trim() || !input.category) { error.textContent = "Term name and category are required."; error.hidden = false; return; } try { const file = form.elements.image.files[0]; if (file) { if (!file.type.startsWith("image/") || file.size > 2 * 1024 * 1024) throw new Error("Choose an image smaller than 2 MB."); input.image = await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.onerror = reject; reader.readAsDataURL(file); }); } else input.image = form.dataset.currentImage || null; editingId ? await updateCustomTerm(editingId, input) : await addCustomTerm(input); dialog("#term-form-dialog").close(); await afterChange(editingId ? "Term updated." : "Term added."); } catch (err) { error.textContent = err.message || "Could not save this term."; error.hidden = false; } }
