...
Search is instant and typo tolerant; use category filters, Favorites, or A–Z mode to browse. Select a card for details and related-term links. **Add Term** creates a device-local custom entry; only custom entries can be edited or deleted. Backup & Restore exports or imports those custom entries as JSON. The status strip reports connection state, the fixed `100/100` official cache count, custom count, and category. Custom data is not synced or remotely backed up unless you export it.
